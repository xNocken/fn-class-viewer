import type { Function, PropertyType } from "$lib/server/get-classes";
...
const getMemCrc = (buffer: Uint8Array, crc = 0) => {
  let crcCopy = (~crc >>> 0) >>> 0;
  let { length } = buffer;
  let index = 0;
  const data4 = new Uint32Array(buffer.buffer, buffer.byteOffset, Math.floor(length / 4));

  for (let repeat = Math.floor(length / 8); repeat; repeat -= 1) {
    const V1 = (data4[index] ^ (crcCopy >>> 0)) >>> 0;
    index += 1;
    const V2 = data4[index] >>> 0;
    index += 1;

    crcCopy = crcTable[7][V1 & 0xFF]
      ^ crcTable[6][(V1 >>> 8) & 0xFF]
      ^ crcTable[5][(V1 >>> 16) & 0xFF]
      ^ crcTable[4][V1 >>> 24]
      ^ crcTable[3][V2 & 0xFF]
      ^ crcTable[2][(V2 >>> 8) & 0xFF]
      ^ crcTable[1][(V2 >>> 16) & 0xFF]
      ^ crcTable[0][V2 >>> 24];
  }
  ...
  for (; length; length -= 1) {
    crcCopy = (crcCopy >>> 8) ^ crcTable[0][((crcCopy >>> 0) & 0xFF) ^ buffer[index]];
    index += 1;
  }
...
const getCStrCrc = (data: string, crc: number) => {
  const chars = data.split('').map((char) => char.charCodeAt(0));
  let crcCopy = (~crc >>> 0) >>> 0;

  for (let i = 0; i < chars.length; i += 1) {
    const char = chars[i];

    crcCopy = ((crcCopy >>> 8) ^ crcTable[0][((crcCopy ^ char) >>> 0) & 0xFF]);
    crcCopy = ((crcCopy >>> 8) ^ crcTable[0][(crcCopy >>> 0) & 0xFF]);
    crcCopy = ((crcCopy >>> 8) ^ crcTable[0][(crcCopy >>> 0) & 0xFF]);
    crcCopy = ((crcCopy >>> 8) ^ crcTable[0][(crcCopy >>> 0) & 0xFF]);
  }
...
const bitsForMaxValue = (maxValue: number) => {
  let bits = 0;
...
const getCppType = (property: PropertyType) => {
...
export const getCmdChecksum = (property: PropertyType, staticArrayIndex = 0, inChecksum = 0) => {
  let checksum = inChecksum;

  checksum = getCStrCrc(property.Name.toLowerCase(), checksum);
  checksum = getCStrCrc(getCppType(property).toLowerCase(), checksum);
  checksum = getMemCrc(new Uint8Array([staticArrayIndex, 0, 0, 0]), checksum);

  if (property.Type === 'EnumProperty' || property.Type === 'ByteProperty') {
    const enumm = property.Enum;

    let bitsRequired = 8;

    if (enumm) {
      bitsRequired = bitsForMaxValue(enumm?.Members.length);
    } else if (property.InnerType) {
      console.error('Enum not found for property', property);
    }

    checksum = getMemCrc(new Uint8Array([bitsRequired, 0, 0, 0, 0, 0, 0, 0]), checksum);
  }
...
export const getFuncChecksum = (func: Function) => {
  let checksum = 0;

  checksum = getCStrCrc(func.Name.toLowerCase(), checksum);
  checksum = getCStrCrc(func.Flags.toString(), checksum);
